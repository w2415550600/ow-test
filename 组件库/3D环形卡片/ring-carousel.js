...
;(function(global) {
  ...
  RingCarousel.prototype._openDetail = function(idx) {
    ...
    var DETAIL_W = this.options.detailWidth;
    var DETAIL_H = this.options.detailHeight;
    ...
    this._flyImg.src = d.image || '';

    // 初始化飞出卡片
    gsap.set(this._flyCard, {
      left: startLeft,
      top: startTop,
      width: startW,
      height: startH,
    ...
    this._flyCard.classList.add('active');

    // 初始化信息面板
    gsap.set(this._detailInfo, {
      left: infoLeft,
      top: infoTop,
      opacity: 0,
    });
    this._detailInfo.classList.add('active');

    // 更新详情内容
    this._detailIndexEl.textContent =
      String(idx + 1).padStart(2, '0') + ' / ' + String(this._cards.length).padStart(2, '0');
    this._detailTitleEl.textContent = d.title || '';
    this._detailDescEl.textContent = d.desc || '暂无详细描述';
    this._pagerIndicator.textContent = (idx + 1) + ' / ' + this._cards.length;

    // 激活遮罩和翻页器
    this._detailOverlay.classList.add('active');
    this._bottomPager.classList.add('active');
    ...
    // 内容逐个出现
    this._detailTl.from(this._detailIndexEl, { y: 10, opacity: 0, duration: 0.3, ease: 'power2.out' }, 0.3);
    this._detailTl.from(this._detailTitleEl, { y: 15, opacity: 0, duration: 0.4, ease: 'power2.out' }, 0.35);
    this._detailTl.from(this._detailDescEl, { y: 15, opacity: 0, duration: 0.4, ease: 'power2.out' }, 0.45);
  };
  ...
  RingCarousel.prototype._detailNavigate = function(dir) {
    ...
    this._detailTl.from(this._detailIndexEl, { y: 8, opacity: 0, duration: 0.25, ease: 'power2.out' }, 0.45);
    this._detailTl.from(this._detailTitleEl, { y: 10, opacity: 0, duration: 0.3, ease: 'power2.out' }, 0.5);
    this._detailTl.from(this._detailDescEl, { y: 10, opacity: 0, duration: 0.3, ease: 'power2.out' }, 0.58);
  };
